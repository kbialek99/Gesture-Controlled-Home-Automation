...
public:
    ConnectionManager() : espClient(), http(), client(espClient), camera(Camera::getInstance())
    {
        serverUrl = "http://192.168.0.137:8080/upload";
        logUrl = "http://192.168.0.137:8080/log";
    }
    ConnectionManager(const char *serverUrl, const char *logUrl) : espClient(), http(), client(espClient), serverUrl(serverUrl), logUrl(logUrl) {camera = Camera::getInstance();}
    ...
    inline void cm_startCamera() const
    {
        camera->startCamera();
    }
    inline void cm_stopCamera() const
    {
        camera->stopCamera();
    }
